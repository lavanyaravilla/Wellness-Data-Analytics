...
def scrape_calisthenic(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/calisthenic-gyms"
    ...
    with sync_playwright() as p:
        ...
        # ✅ broader selector
        try:
            page.wait_for_selector("div.resultbox_textbox, div.resultbox_imagebox, div.resultbox_info", timeout=60000)
        except Exception as e:
            print("Timeout waiting for listings:", e)
            print(page.content()[:2000])  # dump HTML for debugging
            browser.close()
            return

        listings = page.locator("div.resultbox_textbox, div.resultbox_imagebox, div.resultbox_info")
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in calisthenic_data and name.strip():
                        calisthenic_data[key] = {
                            "name": name.strip(),
                            "address": address.strip(),
                            "phone": phone.strip(),
                            "rating": rating.strip(),
                            "reviews": reviews.strip(),
                            "category": "Calisthenic-Gym"
                        }
        ...
        # ✅ Save results
        df = pd.DataFrame(list(calisthenic_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_calisthenic.csv", index=False)
        df.to_excel(f"output/justdial_calisthenic_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_calisthenic(scroll_time=60)
