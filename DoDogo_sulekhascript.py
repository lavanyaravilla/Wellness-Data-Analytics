...
def scrape_karate_classes(scroll_time=60):
    url = "https://www.sulekha.com/karate-classes/bangalore"
    ...
    with sync_playwright() as p:
        ...
        # ✅ Karate listings container (Sulekha uses 'resultbox_info' or 'resultbox_textbox')
        page.wait_for_selector("div[class*='resultbox_info'], div[class*='resultbox_textbox']", timeout=60000)
        listings = page.locator("div[class*='resultbox_info'], div[class*='resultbox_textbox']")
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in karate_data and name.strip():
                        karate_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(karate_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/sulekha_karate_classes.csv", index=False)
        df.to_excel(f"output/sulekha_karate_classes_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_karate_classes(scroll_time=60)
