...
def scrape_kickboxing(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/kickboxing-studio"
    ...
    with sync_playwright() as p:
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in kickboxing_data and name.strip():
                        kickboxing_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(kickboxing_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_kickboxing.csv", index=False)
        df.to_excel(f"output/justdial_kickboxing_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_kickboxing(scroll_time=60)
