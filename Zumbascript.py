...
def scrape_dance_zumba(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/dance-zumba-studio"
    ...
    with sync_playwright() as p:
        ...
        # ✅ Dance/Zumba listings container
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in dance_data and name.strip():
                        dance_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(dance_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_dance_zumba.csv", index=False)
        df.to_excel(f"output/justdial_dance_zumba_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_dance_zumba(scroll_time=60)
