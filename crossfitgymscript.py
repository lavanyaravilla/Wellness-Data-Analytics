...
def scrape_crossfit(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/crossfit-gyms"
    ...
    with sync_playwright() as p:
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in crossfit_data and name.strip():
                        crossfit_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(crossfit_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_crossfit.csv", index=False)
        df.to_excel(f"output/justdial_crossfit_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_crossfit(scroll_time=60)
