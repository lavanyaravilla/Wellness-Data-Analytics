...
def scrape_yoga_centres(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/Yoga-Centres"
    ...
    with sync_playwright() as p:
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in yoga_data and name.strip():
                        yoga_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(yoga_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_yoga_centres.csv", index=False)
        df.to_excel(f"output/justdial_yoga_centres_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_yoga_centres(scroll_time=60)
