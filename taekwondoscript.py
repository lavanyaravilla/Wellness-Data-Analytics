...
def scrape_taekwondo(scroll_time=60):
    url = "https://www.justdial.com/Bangalore/taekwondo-academy"
    ...
    with sync_playwright() as p:
        ...
        while time.time() - start < scroll_time:
            ...
            for i in range(listings.count()):
                ...
                try:
                    ...
                    if key not in taekwondo_data and name.strip():
                        taekwondo_data[key] = {
                            "name": name.strip(),
        ...
        # ✅ Save results
        df = pd.DataFrame(list(taekwondo_data.values()))
        if not os.path.exists("output"):
            os.makedirs("output")

        df.to_csv("output/justdial_taekwondo.csv", index=False)
        df.to_excel(f"output/justdial_taekwondo_{int(time.time())}.xlsx", index=False)

        browser.close()

if __name__ == "__main__":
    scrape_taekwondo(scroll_time=60)
